app.controller("gameController", function($scope,$http,menuSelected,addToCart){
var vm = this;
vm.addToCart=addToCart;
vm.navSelected='games';
...
$scope.$watch('navSelected', function (newValue, oldValue) {
  ...
  vm.filterProducts=[];   //List containing all Games
  vm.checkedTags=[];
  //----------------Getting List of Games-------------------------------
  var responsePromise=$http({withCredentials: true, method: 'GET', url: "http://localhost:8000/api/games" });
  ...
  //-------------Getting All genres------------------------
  responsePromise = $http.get("http://localhost:8000/api/games/genre");
  responsePromise.success(function(data, status, headers, config) {
    for(i=0;i<data.length;++i){
      vm.genres.push({
        id: data[i].id,
